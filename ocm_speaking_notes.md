...
"OK, this is the picture that really frames the problem. On the left I'm plotting the yield distribution for both datasets, normalised so the size difference doesn't dominate. The blue curve is our 89,000 experiments, the orange curve is the 3,852 literature samples. The dashed lines are the means — ours is at 5.25, literature is at 8.67, and that red shaded band between them is the 3.42 percentage point gap. That gap is not noise. It's caused by literature only publishing the experiments that worked, and by literature using optimised flow rates and gas ratios where we use a fixed setup. So if I just mix the two datasets together, the model is going to see contradictory labels for similar chemistry — that's what this picture is warning us about.

On the right, I'm showing the same datasets but in feature space. I took the 67 features per sample, ran PCA, and plotted the first two components. The blue cloud is our data — it sits in one region of chemical space. The orange dots are literature, and you can see they spread far outside our cluster. That's the covariate shift — most literature samples describe chemistry our lab has never tested. So if I added them all in, I'd be training the model on chemistry it will never see in deployment. This picture is what motivates everything from Chapter 5 onwards — we need a way to filter literature down to what's actually relevant."

**When the top elements figure appears:**

"And just to make the chemistry asymmetry concrete — the PCA is abstract, this isn't. This shows the top 15 elements by usage frequency in each dataset, side by side. You can see the palette is different — our lab leans on a specific set of active phases and promoters, literature has a broader and different mix. So when I say 78% of literature is foreign chemistry, this is what I mean — the literature isn't just our chemistry with different labels, it's genuinely different recipes."
...
**When the DRST score histogram appears:**

"This histogram shows where every one of the 3,852 literature samples lands on the P(ours|x) axis. You can see most of them are bunched at the low end — those are the chemistries the classifier confidently calls foreign. There's a long tail extending toward 1.0 — those are samples that look genuinely similar to our data. The dashed lines are the candidate thresholds I tried. We picked tau=0.30 because it gave the lowest validation error in the sweep, and it retains 782 of the 3852 samples — about 20%.

RMSE dropped to 2.019, so 5.3% better than baseline. But here's what bothered me looking at this histogram: the cutoff is hard. A sample scoring 0.29 is completely thrown away. A sample scoring 0.31 gets full weight. That felt arbitrary, especially since the density is continuous across the threshold. Which led me to ask — can we do something softer?"
...
The result is RMSE 2.035, so about 4.6% better than baseline. Very similar to DRST.

**When the KMM weights + agreement scatter appears:**

"The left panel is the weight distribution — you can see a huge spike near zero. 78.5% of literature samples get a weight below 0.1. So KMM is doing the same thing as DRST — most literature is being effectively discarded — it just does it on a continuous scale.

The right panel is what I want to spend a moment on. I'm plotting every literature sample as a point — the x-axis is its DRST score, the y-axis is its KMM weight. If the two methods agreed perfectly the points would lie on a clean line. They don't — there's spread — but they do trend together with r=0.79. So one method is a logistic classifier, the other is a quadratic optimisation, two completely different families of math — and they're both independently flagging the same ~78% of samples as low-value. That gives me some confidence that it's a real signal about the data, not an artifact of either method.

But here's what both DRST and KMM still have in common: they're adding literature labels directly into training. Those labels have a 3.42 percentage point systematic offset — the gap we saw in the very first plot. We're reducing the damage by filtering, but we're not eliminating the root cause. That's what the next method does."
...
"Before we get to SHAP, just one slide to put the whole story together. This bar chart has all five methods, sorted by RMSE. The dashed line is the baseline — anything to the right of it harmed the model, anything to the left improved it. You can see the red bar — naive merge — is the only one that crosses to the wrong side. Then DRST and KMM both move us slightly to the left, in roughly the same place. And then Prior Feature Transfer at the top is the biggest jump — by a meaningful margin. So the picture of the analysis is: don't blindly add data, filter what you add, but ideally don't add literature as labels at all — use it as a prior signal."
...
The most reassuring thing is that lit_prior_prediction comes out as the number one feature — it's at the top of the y-axis, with the widest spread. The transfer learning is genuinely being used, not just sitting there as dead weight. Then temperature is second — and you can see the red dots, high temperature, are on the right side, pushing yield up, which is consistent with OCM chemistry. Then Ba, Mn, La, Ce all have positive effects when their loadings are high, which again matches what we know about these elements as OCM active phases and promoters.
